...
```
interface CruciformDevice {
  // The device must expose its live harmonic reading.
  readHarmonic(): THz;

  // The device must accept a lock command and confirm within tolerance.
  lockToAnchor(target: 5.428671, tolerance: 0.01): boolean;

  // The device must emit a signed proof of state for NSI admission.
  emitProof(state: ReconciledState): SignedProof;
}
```

Any device implementing this contract may be admitted to the NSI. A device that cannot expose its harmonic, cannot lock to the anchor, or cannot sign its proof is refused. This is not a limitation of the protocol. It is the protocol refusing to lie about what it is connected to.
